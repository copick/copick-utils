...
def clipseg(
    ...
    # Create selector for input segmentations
    selector = ConversionSelector(
    ...
    if not all_tasks:
        logger.warning("No matching segmentation/reference pairs found for distance limiting")
        return

    logger.info(f"Found {len(all_tasks)} tasks across {len(runs_to_process)} runs")

    results = limit_segmentation_by_distance_batch(
        root=root,
        conversion_tasks=all_tasks,
        run_names=run_names_list,
        workers=workers,
