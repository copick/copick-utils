...
from copick_utils.cli.util import (
    ...
    add_reference_mesh_options,
    add_reference_segmentation_options,
    add_workers_option,
)
...
def picksin(
    ...
    logger.info(f"Including picks inside reference volume for object '{pick_object_name}'")
    ...
    if not all_tasks:
        logger.warning("No matching picks/reference pairs found for inclusion filtering")
        return

    logger.info(f"Found {len(all_tasks)} tasks across {len(runs_to_process)} runs")

    results = picks_inclusion_by_mesh_batch(
        root=root,
