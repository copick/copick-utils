...
class TestInputOutputSelector:
    ...
    def test_one_to_one_selection(self, temp_copick_project):
        ...
        assert selector.is_pattern is False
        assert selector.get_mode_description() == "One-to-one: exact session ID mapping"

        tasks = selector.get_conversion_tasks(run)
        assert len(tasks) == 1

        task = tasks[0]
        assert task.input_pick_object_name == "sphere-points"
        assert task.input_pick_user_id == "test"
        assert task.input_pick_session_id == "single"
        assert task.output_mesh_object_name == "test-mesh"
        assert task.output_mesh_user_id == "test-user"
        assert task.output_mesh_session_id == "output-single"
    ...
    def test_one_to_many_selection(self, temp_copick_project):
        ...
        assert selector.is_pattern is False
        assert selector.get_mode_description() == "One-to-many: single input to template output with {instance_id}"

        tasks = selector.get_conversion_tasks(run)
        assert len(tasks) == 1

        task = tasks[0]
        assert task.output_mesh_session_id == "output-{instance_id}"
    ...
    def test_many_to_many_selection(self, temp_copick_project):
        ...
        assert selector.is_pattern is True
        assert (
            selector.get_mode_description()
            == "Many-to-many: regex pattern input to template output with {input_session_id} and optional {instance_id}"
        )

        tasks = selector.get_conversion_tasks(run)
        assert len(tasks) == 3  # Should match sphere-0, sphere-1, sphere-2

        session_ids = [task.output_mesh_session_id for task in tasks]
    ...
    def test_default_object_name(self, temp_copick_project):
        ...
        tasks = selector.get_conversion_tasks(run)
        assert len(tasks) == 1
        assert tasks[0].output_mesh_object_name == "sphere-points"

    def test_default_user_id(self, temp_copick_project):
        """Test default user_id behavior."""
        root, temp_path = temp_copick_project
        run = root.get_run("test_run")
        ...
        selector = InputOutputSelector(
            ...
            pick_session_id="single",
            mesh_object_name="test-mesh",
            mesh_user_id=None,  # Should use default
            mesh_session_id="output-single",
            individual_meshes=False,
        )

        # Set default user_id
        selector.set_default_user_id("default-user")

        tasks = selector.get_conversion_tasks(run)
        assert len(tasks) == 1
        assert tasks[0].output_mesh_user_id == "default-user"
    ...
    def test_session_id_template_validation(self):
        ...
        selector._validate_session_id_template()

        # Test with individual meshes - should require instance_id
        selector.individual_meshes = True
        selector.mesh_session_id = "output-{input_session_id}-{instance_id}"
        selector._validate_session_id_template()

        # Test missing input_session_id for pattern
        selector.mesh_session_id = "output-{instance_id}"
        with pytest.raises(ValueError, match="input_session_id"):
            selector._validate_session_id_template()

    def test_conversion_task_attributes(self, temp_copick_project):
        """Test ConversionTask attributes."""
        root, temp_path = temp_copick_project
        run = root.get_run("test_run")
        ...
        assert task.input_pick_user_id == "test"
        assert task.input_pick_session_id == "single"
        assert task.output_mesh_object_name == "output-mesh"
        assert task.output_mesh_user_id == "output-user"
        assert task.output_mesh_session_id == "output-session"

        # Test string representation
        task_str = str(task)
        assert "test_run" in task_str
        assert "sphere-points" in task_str
        assert "test/single" in task_str
        assert "output-mesh" in task_str
        assert "output-user/output-session" in task_str
...
class TestValidatePlaceholders:
    ...
    def test_input_session_id_with_exact_match(self):
        """Test input_session_id placeholder with exact match."""
        with pytest.raises(ValueError, match="input_session_id.*not.*pattern"):
            validate_placeholders("exact", "output-{input_session_id}", False)

    def test_instance_id_without_individual(self):
        """Test instance_id placeholder without individual meshes."""
        with pytest.raises(ValueError, match="instance_id.*individual_meshes"):
            validate_placeholders("pattern-.*", "output-{input_session_id}-{instance_id}", False)


class TestComplexScenarios:
    """Test complex selection scenarios."""

    def test_multiple_runs_pattern_matching(self, temp_copick_project):
        ...
        input_sessions = {task.input_pick_session_id for task in tasks}
        output_sessions = {task.output_mesh_session_id for task in tasks}
    ...
    def test_combined_clustering_and_individual_meshes(self, temp_copick_project):
        ...
        task = tasks[0]
        assert task.output_mesh_session_id == "cluster-{instance_id}"
        assert "{instance_id}" in task.output_mesh_session_id  # Template should be preserved
    ...
    def test_object_name_inheritance(self, temp_copick_project):
        ...
        # Test with None mesh_object_name
        selector = InputOutputSelector(
            pick_object_name="sphere-points",
            pick_user_id="test",
            pick_session_id="single",
        ...
        tasks = selector.get_conversion_tasks(run)
        assert len(tasks) == 1
        assert tasks[0].output_mesh_object_name == "explicit-mesh"

    def test_edge_case_empty_pattern(self, temp_copick_project):
        """Test edge case with pattern that has no special characters."""
        root, temp_path = temp_copick_project
        root.get_run("test_run")

        # Pattern with no regex special chars should still work as pattern
        selector = InputOutputSelector(
            pick_object_name="sphere-points",
            pick_user_id="test",
            pick_session_id="single",  # No regex chars, but has placeholders in output
            mesh_object_name="test",
            mesh_user_id="test",
            mesh_session_id="output-{input_session_id}",  # This makes it pattern mode
            individual_meshes=False,
        )

        # This should be detected as pattern mode due to {input_session_id}
        with pytest.raises(ValueError, match="input_session_id.*not.*pattern"):
            selector._validate_session_id_template()
